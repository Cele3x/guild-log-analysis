...
class OneArmedBanditAnalysis(BossAnalysisBase):
    ...
    def analyze(self, report_codes: List[str]) -> None:
        ...
        logger.info(
            f"Starting One-Armed Bandit analysis for {len(report_codes)} reports"
        )
    ...
    def _process_report(self, report_code: str) -> None:
        ...
        interrupts_data = self._analyze_overload_interrupts(
            report_code, fight_ids, report_players
        )
        report_results["analysis"].append(
            {"name": "Overload! Interrupts", "data": interrupts_data}
        )

        uptime_data = self._analyze_high_roller_uptime(
            report_code, fight_ids, report_players
        )
        report_results["analysis"].append(
            {"name": "High Roller Uptime", "data": uptime_data}
        )

        damage_data = self._analyze_damage_to_premium_dynamite_booties(
            report_code, fight_ids, report_players
        )
        report_results["analysis"].append(
            {"name": "Premium Dynamite Booties Damage", "data": damage_data}
        )

        reel_assistants_data = self._analyze_damage_to_reel_assistants(
            report_code, fight_ids, report_players
        )
        report_results["analysis"].append(
            {"name": "Reel Assistants Damage", "data": reel_assistants_data}
        )

        boss_damage_data = self._analyze_damage_to_boss(
            report_code, fight_ids, report_players
        )
        report_results["analysis"].append(
            {"name": "Boss Damage", "data": boss_damage_data}
        )

        absorbed_damage_data = self._analyze_absorbed_damage_to_reel_assistants(
            report_code, fight_ids, report_players
        )
        report_results["analysis"].append(
            {"name": "Absorbed Damage to Reel Assistants", "data": absorbed_damage_data}
        )

        self.results.append(report_results)
        logger.info(
            f"Successfully processed report {report_code} with {len(report_results['analysis'])} analyses"
        )

    def _analyze_overload_interrupts(
        self, report_code: str, fight_ids, report_players
    ) -> List:
    ...
    def _analyze_high_roller_uptime(
        self, report_code: str, fight_ids, report_players
    ) -> List:
    ...
    def _analyze_damage_to_premium_dynamite_booties(
        self, report_code: str, fight_ids, report_players
    ) -> List:
    ...
    def _analyze_damage_to_reel_assistants(
        self, report_code: str, fight_ids, report_players
    ) -> List:
    ...
    def _analyze_absorbed_damage_to_reel_assistants(
        self, report_code: str, fight_ids, report_players
    ) -> List:
        ...
        # Rename the 'damage' field to match the expected column name
        for player_data in damage_data:
            player_data["absorbed_damage_to_reel_assistants"] = player_data.pop(
                "damage"
            )
    ...
    def generate_plots(self) -> None:
        ...
        sorted_reports = sorted(
            self.results, key=lambda x: x["starttime"], reverse=True
        )
        latest_report = sorted_reports[0]
        report_date = datetime.fromtimestamp(latest_report["starttime"]).strftime(
            "%d.%m.%Y"
        )

        # Get fight counts for current and previous reports
        current_fight_count = len(latest_report.get("fight_ids", []))
        previous_fight_count = (
            len(sorted_reports[1].get("fight_ids", []))
        ...
        self._generate_interrupts_plot(
            report_date, current_fight_count, previous_fight_count
        )
    ...
    def _generate_uptime_plot(self, report_date: str) -> None:
        ...
        current_hr_data, previous_hr_dict = self.find_analysis_data(
            "High Roller Uptime", "uptime_percentage", "player_name"
        )
    ...
    def _generate_reel_assistants_damage_plot(
        self, report_date: str, current_fight_count: int, previous_fight_count: int
    ) -> None:
        """Generate plot for Reel Assistants damage."""
        current_dmg_data, previous_dmg_dict = self.find_analysis_data(
            "Reel Assistants Damage", "damage_to_reel_assistants", "player_name"
        )
    ...
    def _generate_boss_damage_plot(
        self, report_date: str, current_fight_count: int, previous_fight_count: int
    ) -> None:
        """Generate plot for Boss damage."""
        current_dmg_data, previous_dmg_dict = self.find_analysis_data(
            "Boss Damage", "damage_to_boss", "player_name"
        )
    ...
    def _generate_absorbed_damage_plot(
        self, report_date: str, current_fight_count: int, previous_fight_count: int
    ) -> None:
